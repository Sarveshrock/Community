...
/// A team's private group chat (spec section 4). Unlike 1-to-1 [ChatScreen],
/// every bubble here needs to say *who* sent it — access itself is enforced
/// server-side (RLS on `messages`/`conversations` via `is_conversation_member`,
/// see migration 0028), so a conversation id resolving to null here means
/// either the chat hasn't been created yet or the viewer truly isn't a
/// member — both render as a clear empty/permission state rather than an
/// error.
class TeamChatScreen extends ConsumerStatefulWidget {
  const TeamChatScreen(
      {super.key, required this.teamRequirementId, this.teamName});
...
class _TeamChatScreenState extends ConsumerState<TeamChatScreen> {
  ...
  Future<void> _pickAndSendImage(
    ...
    await _uploadAndSend(conversationId,
  ...
  Future<void> _pickAndSendFile(String conversationId) async {
    ...
    final type = _imageExtensions.contains(extension)
        ...
        : MessageType.file;
    await _uploadAndSend(conversationId,
        bytes: bytes, fileName: file.name, type: type);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final conversationIdAsync =
        ref.watch(teamConversationIdProvider(widget.teamRequirementId));
    final teamAsync = ref.watch(teamDetailProvider(widget.teamRequirementId));
    final membersById = <String, TeamMemberInfo>{
      for (final m
          in teamAsync.valueOrNull?.members ?? const <TeamMemberInfo>[])
        m.profileId: m,
    };
...
class _TeamMessageBubble extends ConsumerWidget {
  const _TeamMessageBubble(
      {required this.message, required this.isMine, this.sender});

  final Message message;
  final bool isMine;
  final TeamMemberInfo? sender;
        ...
        child: GestureDetector(
          ...
          child: Column(
            crossAxisAlignment:
                isMine ? CrossAxisAlignment.end : CrossAxisAlignment.start,
            children: [
              // Sender identification (spec: "See which team member sent
              // each message") — only shown for incoming messages, and only
              // when it changes from the previous one, WhatsApp-group style.
              if (sender != null)
                Padding(
